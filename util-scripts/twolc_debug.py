...
parser.add_argument('-a', '--showall', dest='showall', action='store_true', help='Show all forms created and excluded by each rule')
parser.add_argument('-p', '--process', dest='process', action='store_true', help='Just process and show output form')
...
def get_output_of_all_rules(form, twolcFile):
...
def process_form(inputForm, twolcFile):
	...
	p2 = Popen(["hfst-strings2fst", "-S"], stdin=p1.stdout, stdout=PIPE)
	p3 = Popen(["hfst-compose-intersect", twolcFile], stdin=p2.stdout, stdout=PIPE)
...
if args.form != None and args.twolcFile != None:
	for form in args.form:
		...
		main_loop(args.twolcFile[0], form, args.correct, args.showall, args.process)

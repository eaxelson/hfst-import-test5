...
\begin{abstract}
    ...
    decrease in the size of finite-state networks without drastic
...
finite-state networks which result from lexicographical-morphological
...
morpheme structure which is implicitly present in lexical
descriptions.
...
in optimisation~\shortcite{karttunen2006numbers}. In this paper, he
...
\texttt{Root}. If later in the path occurs flag R that requires test
...
is much slower for transducers that contain automatic flags, lookup speed of 1000 words per second is sufficient for various applications with transducers.
...
\verb+@P.LEXNAME.IV@+, but they can't collapse into the same path. It
should be checked if skipping inserting flags for those paths would
...
to maintain linguistic descriptions.  \iffalse
\section*{Acknowledgments}
The research leading to these results has received funding from the
European Commission’s 7th Framework Program under grant agreement
n°238405 (CLARA).  \fi

...
\begin{verbatim}
$ echo '[ ? -  $[ @"FrontV" ] | hfst-regexp2fst > Example
\end{verbatim}
...
Left-to-right longest match is supposed to give exactly one output string for each input string. However, when compiled using XFST, both of the following left-to-right longest match rules result in transducers which for input \verb+aabbaax+ give two outputs: \verb+aaxx+ and \verb+xxx+.
\begin{verbatim}
regex a+ b+ | b+ a+ @-> x \\ _ x ;
regex a+ b+ | b+ a+ @-> x \/ _ x ;
\end{verbatim}

The same regular expression compiled with \verb+hfst-regexp2fst+ tool
...
\textbf{minus}.
Therefore, if upper commands were run in \verb+hfst-regexp2fst+ tool, the
resulting alphabet would always be \verb+a, b+.

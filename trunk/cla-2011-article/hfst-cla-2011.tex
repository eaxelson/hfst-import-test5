...
\item {\bf Projection $\mathrm{P}(T)$:} A unary operation, whose result
  ...
  \end{equation}
\item {\bf scaling $\mathrm{W}_k(T)$:} A unary operation, whose result accepts the same correspondences $s_i\mathrm{:}s_o$ as $T$ with weight $k\cdot \mathrm{w}_T(s_i\mathrm{:}s_o)$. Here we use regular multiplication of floating point numbers (not $\otimes$).

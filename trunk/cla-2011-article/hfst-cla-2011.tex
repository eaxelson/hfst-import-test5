...
\item {\bf scaling $\mathrm{W}(k,T)$:} A unary operation, whose result accepts the same correspondences $s_i\mathrm{:}s_o$ as $T$ with weight $k\cdot w_T(s_i\mathrm{:}s_o)$. Here we use regular multiplication of floating point numbers (not $\otimes$).
...
$$w_{S-T}(s_i\mathrm{:}s_o) < \infty \Rightarrow w_{S-T}(s_i\mathrm{:}s_o) = w_{S}(s_i\mathrm{:}s_o)$$
\end{itemize}
...
and give it weight $w_{L_O}(s\mathrm{:}s)$, if it is accepted by $L_O$
and $w_{max}$ otherwise. The same applies for $L_I \cup U_{max}$.

We want to construct such a transducer $L$, that $w_L(s\mathrm{:}s)$
is a weighted average of the weights $w_{L_I \cup U_{max}}(s\mathrm{:}s)$ and
$w_{L_O \cup U_{max}}(s\mathrm{:}s)$, i.e.
$$w_L(s\mathrm{:}s) = k\cdot w_{L_I}(s\mathrm{:}s) + (1 - k)\cdot w_{L_O}(s\mathrm{:}s)\text{, where }0\leq k \leq 1\text{.}$$
...
$$ L = \mathrm{W}\big(k,(L_I \cup U_{max})\big) \circ \mathrm{W}\big(1-k,(L_O \cup U_{max})\big)$$

...
result is the transducer in Figure~\ref{fig:replace_compose}, shown in ATT format. First two columns show state transitions, second two input and output symbols. Final column shows weight for each transition. If some state is final, it is printed alone in a row, followed only by its weight. The result transducer in  Figure~\ref{fig:replace_compose} has 5 states (0 -- 4), and only state 4 is final. Although the default output of
\verb!hfst-regexp2fst! is weighted, weighted replace rules are not suported yet. Therefore, all the weights get value 0.0 when
compiled.
...
For example, when the \ \verb!//!\ sign is used as context direction operator, 
...
In XFST, all rules in a parallel rule expression have to share the same template - 
they need to have the same arrow and same context format.
...
would not be allowed in XFST because one rule has a context, and another one doesn't, but is valid in HFST.
\begin{figure}
{\footnotesize
\begin{verbatim}
...
\caption{Parallel rules not sharing the same context template}

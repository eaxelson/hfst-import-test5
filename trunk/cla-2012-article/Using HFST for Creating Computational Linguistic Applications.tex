...
Lexicon files for the lexicon compiler \textbf{hfst-lexc} describe the morphotax of a
...
Using an extension to the xerox two-level rule syntax in \textbf{hfst-twolc},
it is possible to formulate rules with negative context, i.e. context
which prohibit the triggering of the rule. 
Figure~\ref{negative-context-rule} shows a schematic example of
...
<[ Z:y Z:x ]> <==> a _ a ; 
\end{verbatim}
\caption{Rule with regular expression center and two auxiliary rules which}\label{regular-center}
\end{figure}

Regular expression centers allow rules to govern longer segments than
single symbols, e.g. see Figure~\ref{regular-center}. This can be useful 
when describing languages with complex allomorphy.
...
described in \cite{Kempe96parallelreplacement} and 
the Finite State Morphology book \cite{beesley/2003}. 
Although they share the same notation, the HFST replace rules were developed 
from the concept of Generalized Lenient Composition, 
described in \cite{ylijyra/2008b}. 

In this paper section, we will describe how to use Replace Rules with HFST tools. 
...
where A and B are regular expressions that describe languages, 
states that A in the upper language maps to B in the lower language.

Replace rules can be compiled into transducer using the HFST tool 
\textbf{hfst-regexp\-2fst}. This tool takes a regular expresion as input 
and gives a corresponding transducer written in a binary file as output. 
To convert transducers from binary format to text format, 
there is an HFST tool \textbf{hfst-fst2txt}. Therefore, the upper rule 
...
It has 5 states (0 -- 4), and only state 4 is final. The default output of \textbf{hfst-regexp2fst} is 
the transducer format \textbf{tropical\_openfst}, which supports weights. 
Since weights are not used in replace rules, the weights get value 0.000000 when compiled. 
...
Popular formalisms for describing the morpotax of a language tend to be some variation 
...
dictionaries, but also morphological analyzers and spell-checkers is
demonstrated in \cite{pirinen/2012/lrec}.\footnote{The examples are also available at

...
void iface_apply_file(char *infilename, char *outfilename, int direction) {
    ...
    while ((fgets(inword,1024,INFILE)) != NULL) {
        ...
        fprintf(OUTFILE,"\n%s\n", inword);
        if (direction == AP_D)
            result = apply_down(inword);
        else
            result = apply_up(inword);
...
void iface_compose() {
    if (iface_stack_check(2)) {
        while (stack_size()>1) 
            stack_add(fsm_topsort(fsm_minimize(fsm_compose(stack_pop_q(),stack_pop_q()))));
    }
}
...
void iface_conc() {
    if (iface_stack_check(2)) {
        while (stack_size()>1) 
            stack_add(fsm_topsort(fsm_minimize(fsm_concat(stack_pop_q(),stack_pop_q()))));
    }
}

void iface_crossproduct() {
    if (iface_stack_check(2)) {
        stack_add(fsm_topsort(fsm_minimize(fsm_cross_product(stack_pop_q(),stack_pop_q()))));
    }
}
...
void iface_ignore() {
    if (iface_stack_check(2)) {
        stack_add(fsm_topsort(fsm_minimize(fsm_ignore(stack_pop_q(),stack_pop_q(),OP_IGNORE_ALL))));
    }
}
...
void iface_lower_words(int limit) {
    ...
    if (iface_stack_check(1)) {
        apply_init(stack_find_top()->fsm);
        for (i = limit; i > 0; i--) {
            result = apply_lower_words();
            if (result == NULL)
                break;
            printf("%s\n",result);
        }
...
static int view_net(struct fsm *net) {
  ...
  sprintf(tmpstr,"gqview %s &",pngname);
  if (system(tmpstr) == -1)
      printf("Error opening viewer.\n");
  xxfree(pngname);
#endif /* __APPLE__ */

...
- new command line tools: hfst-prune-alphabet, hfst-xfst, hfst-tagger
- flag diacritic harmonization: now identity and unknown symbols are
  not expanded by flag diacritics
- harmonization parameter in binary operators
- support for default symbol

- dependencies: zlib, ncurses used if termcap is not found (hfst-xfst)
...
- all back-ends (SFST, OpenFst and foma) are bundled with HFST, there is
  no more need to install them separately.
...
- added the option print-separator in hfst-fst2strings to achieve better
  control of pipelined command line tools

- function shuffle added under HfstTransducer and hfst-shuffle

- conversions between transducer formats are more efficient

- all binary command line tools handle the case where the first input
  contains several transducers and the second input exactly one transducer
...
- examples from Finite-State Morphology Homepage by Beesley and Karttunen
- via XFST scripts and using HFST command line tools (mainly hfst-regexp2fst)
- how many commands are implemented in hfst-xfst
- other test cases?

...
\maketitle
\begin{abstract}
Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam lectus metus, ultrices at arcu et, blandit hendrerit nunc. Etiam imperdiet porttitor tellus non dapibus. Cras vulputate viverra rutrum. Nam tristique dui sem, eget mattis eros pulvinar non. Integer consectetur luctus porttitor. Nunc in purus cursus arcu sollicitudin consequat ac sed nisi. In pharetra nec leo a congue. Donec non sapien sapien. Etiam at dictum mauris, eget sodales tellus. Ut bibendum augue bibendum, porta lorem eu, placerat magna. Duis tristique iaculis lorem, vel rhoncus magna bibendum et. Pellentesque et magna et odio suscipit fermentum. Phasellus id molestie est, et vulputate leo. Integer venenatis at massa ullamcorper aliquet. Aenean egestas fringilla massa, a bibendum dolor. Pellentesque sapien lectus, malesuada ut iaculis eu, tempus et tellus. - max 200 words
\end{abstract}
...
to semi-automatically introduce flag diacritics into a finite-state
...
but with fully automated process. Moreover, the approach presented in this
...
\begin{figure*}[!htb]
    \includegraphics[width=\textwidth]{transducer.png}
     \caption{Simplified part of Finnish \texttt{lexc} grammar description with automatic flags
...
using a syntax called \texttt{lexc} and expressing morphophonological
...
of inducing flag diacritics into \texttt{lexc} lexicons. Our work can be seen
...
\texttt{operator} is one of the available operators (\verb+P+, \verb+U+, \verb+R+, \verb+D+, \verb+N+, \verb+C+), \texttt{feature} is the name of a feature set by the user and \texttt{value} can be any value held in a feature, also provisionally defined. For additional information on the semantics of flag diacritic operators, see~\newcite{beesley2003finite}.
...
\texttt{Root}. If later in the path there is an \verb+R+ flag that requires test
...
we use $\Sigma^*$ to define universal language and $F$ to define set of all hyper-minimization \verb+P+ and \verb+R+ flags.
...
the map of regular expression keys and matching transducers. The key is added to the trie in the same way as the strings are, i.e. the trie entry will start with 
...
where $f$ is function that transforms trie into a finite state transducer. Transformation of the trie to a transducer is a fast operation, 
...
\section{Results}
\label{sec:results}
In Table~\ref{table:sizes}, we compare the sizes of morphological analyzers built with different approaches. The first column shows analyzer 
size without any flag diacritics. It is followed by sizes reported in~\newcite{drobac2014} for both automatically inserted flag diacritics 
and manually optimized result. Finally there are sizes when 
...
In Table~\ref{table:lookup} we show lookup speed of the morphological analyzers. First, there is the speed of original analyzer, 
followed by the reported speed of the optimal flagged analyzer in ~\newcite{drobac2014} and finally the speed of the analyzer build with our method.
...
allowing users to compose lexical transducer to phonological rules without path pruning afterwards, which makes the whole 
compilation process easier and faster.
In future work, it would be interesting to further optimize our method by automatically recognizing redundant flag diacritics 
and immediately removing them during \texttt{lexc} compilation. It is possible that the smaller number of flag diacritics would 
...
handling of the hyper-minimizing flags, which would result with faster applications.

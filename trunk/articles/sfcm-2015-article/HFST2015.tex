...
Here the only correct tokenization is \mbox{``superhuman''} rather than
\mbox{``super''}
and \mbox{``human''}, but a dictionary would miss this possibility. However,
...
case and number) for each word in a text. It can be contrasted with
POS tagging where the task is to infer the correct part-of-speech of
each word.
...
Lage label sets create a data sparsity problem. For example, for a
second order model and a label set of 1,000 distinct label types, an
...
label unigrams are rare in a sense -- there is necessarily going to be
a lot of label unigrams that occur only a couple of times in the
...
congurence in isolation of the full labels.

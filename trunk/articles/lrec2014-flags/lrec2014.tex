...
\abstract{Flag diacritics, which are special multi-charachter symbols executed at runtime, enable optimising finite-state networks by
    ...
    speed. The algorithm can be applied to any language description, where the biggest achievements are expected in large and complex morphologies. The most noticeable reduction in size we got with Greenlandic transducer, whose original transducer size is on average about 15 times larger than other morphologies. With presented hyper-minimization method, the transducer is reduced to 10,1\% of the original transducer size, with lookup speed decreased only by 9,5\%. 
%    ... (150-200 words)

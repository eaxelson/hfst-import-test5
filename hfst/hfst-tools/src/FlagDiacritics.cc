...
void FlagDiacriticTable::split_diacritic(short diacritic_number,
					 const std::string &diacritic_string)
{
  switch (diacritic_string.at(1))
    {
    case 'P':
      diacritic_operators[diacritic_number] = Pop;
      break;
    case 'N':
      diacritic_operators[diacritic_number] = Nop;
      break;
    case 'D':
      diacritic_operators[diacritic_number] = Dop;
      break;
    case 'R':
      diacritic_operators[diacritic_number] = Rop;
      break;
    case 'C':
      diacritic_operators[diacritic_number] = Cop;
      break;
    case 'U':
      diacritic_operators[diacritic_number] = Uop;
      break;
    default:
      assert(false);
    }
  ...
  if (second_full_stop_pos == std::string::npos)
	...
	diacritic_string.substr(first_full_stop_pos+1,
      ...
      diacritic_values[diacritic_number] = 
...
FlagDiacriticTable::FlagDiacriticTable(void):
  error_flag(false)
{}
bool FlagDiacriticTable::is_diacritic(short key_number)
{ return diacritic_operators.find(key_number) != diacritic_operators.end(); }
...
void FlagDiacriticTable::insert_number(short key_number)
{
  if (is_diacritic(key_number))
    {
      switch (diacritic_operators[key_number])
	{
	case Pop:
	  set_positive_value(diacritic_features[key_number],
			     diacritic_values[key_number]);
	  break;
	case Nop:
	  set_negative_value(diacritic_features[key_number],
			     diacritic_values[key_number]);
	  break;
	case Dop:
	...
	case Cop:
	  ...
	  break;
	case Uop:
	  unify(diacritic_features[key_number],
		diacritic_values[key_number]);
	  break;
	default:
	  assert(false);
...
bool FlagDiacriticTable::is_valid_string(const KeyVector * input_vector)
{
  reset();
  for (KeyVector::const_iterator it = input_vector->begin();
       it != input_vector->end();
       ++it)
    {
      insert_number(*it);
...
KeyVector * FlagDiacriticTable::filter_diacritics(KeyVector * input_vector)
{
  if (not is_valid_string(input_vector))
    {
      delete input_vector;
      return NULL;
    }
  KeyVector * filtered = new KeyVector;
  for (KeyVector::iterator it = input_vector->begin();
       it != input_vector->end();
       ++it)
    {

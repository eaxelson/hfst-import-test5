...
class SymbolPairNumbering {
 ...
 public:
  ...
  SymbolPairNumbering(TransduceR * tr, Alphabet &tr_alphabet):
    next_dense(1),
    next_number(1)
  {
    ...
    for ( Alphabet::const_iterator it = tr_alphabet.begin();
	  it != tr_alphabet.end();
	  ++it )
      get_pair_number(*it);
  }
...
class RuntimeState {
 ...
 public:
  ...
  RuntimeState( StateId n, SymbolPairNumbering * pair_numbers, vector<RuntimeTransition*> &transducer_transitions ):
    transducer_state(n),
  ...
  static void set_transducer(TransduceR * tr)
    { transducer = tr; }
};
...
class RuntimeHeader {
 ...
 public:
  RuntimeHeader( TransduceR * t ) :
    ...
    determinism(NO), // Need to fix!
    minimality(NO),  // Need to fix!
    weightedness(YES),
    cyclicity(YES) // Need to fix!
    {}
...
class TransitionIndexTable {
 ...
 private:
  ...
  void empty_write(FILE * f)
    { 
      size_t foo = fwrite(&EMPTY_SYMBOL_NUMBER,sizeof(EMPTY_SYMBOL_NUMBER),1,f);
      foo = fwrite(&EMPTY_TARGET,sizeof(EMPTY_TARGET),1,f);
    }
  ...
  void add_state( RuntimeState * s,   map<StateId,transition_number> &transducer_nodes);
...
class RuntimeTransitionIndexTable {
 ...
 public:
  ...
  map<StateId,transition_number> &get_transducer_nodes( void )
...
class RuntimeTransducerWriter {
 ...
 private:
  ...
  FILE * output_file;
  Alphabet &regular_transducer_alphabet;
 ...
 public:
  ...
  RuntimeTransducerWriter( TransduceR * tr, Alphabet &tr_alphabet, FILE * f):
    regular_transducer(tr),
    output_file(f),
    regular_transducer_alphabet(tr_alphabet),
    transducer_header(tr),
    symbol_pair_numbers(tr,tr_alphabet),
    transition_index_table(symbol_pair_numbers)
    {
      RuntimeState::set_transducer(tr);

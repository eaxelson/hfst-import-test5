...
\maketitle \begin{abstract} In this paper we inspect the effects of various
finite-state language and error models on the speed of finite-state
spell-checking for case of contextless correction of non-word errors. We
evaluate the speed and memory consumption of full-scale real-world finite-state
spell checkers for three languages covering the broadest available spectrum of
morphological complexity; English, Finnish and Greenlandic. We evaluate the
effects of different finite-state error models and optimisations against
contemporary software-driven spell-checkers with same or equivalent
spell-checking features.  \end{abstract}
...
HFST\footnote{\url{http://hfst.sf.net}}. There is no free software for us as a
...
\newcite{damerau/1964}. It maps four
...
described here is greater than 1 word per second (i.e. 10000 seconds per 10000
words).
...
dictionary instead is usable for morphologically complex language. Furthermore
we showed that the error models can be automatically optimised in several ways
to gain some speed at cost of recall. We noticed that the often unfounded
assumption of leaving the first letter of the word form unmodified while
correcting the word form may cause the least drop in the recall figures even
in Finnish language, and it does gain significant improvement in the correction
speed.

We showed that the memory consumption of the spell-checking process is mainly
unaffected by the selection of error model to the extent as the error model
doesn't generate measurably greater set of corrections per word, which means
that error models can be more freely changed in real world applications as
needed.

In this research we focused on differences in automatically generated error
models and their optimisations, in case of morphologically complex languages.
For future research we intent to study more realistic error models induced from
actual error corpora (e.g. \newcite{brill/2000}). Also we need to research
different statistical versions of the language models, as well as further use
of context in finite-state spell-checking (as in
\newcite{pirinen/2012/cicling}).

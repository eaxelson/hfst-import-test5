...
for i in .sfst .ofst .foma ; do
    ...
    if test -f cat.hfst$i ; then
        if ! ../src/hfst-lexc2fst $FFLAG $srcdir/cat.lexc > test.hfst ; then
    ...
    for f in $LEXCTESTS ; do
        if ! ../src/hfst-lexc2fst $FFLAG $srcdir/$f > test.hfst ; then
            echo lexc2fst $FFLAG $f failed with $?
            exit 1
        fi
        rm test.hfst
    done
    if ! ../src/hfst-lexc2fst $FFLAG $srcdir/basic.multi-file-1.lexc \

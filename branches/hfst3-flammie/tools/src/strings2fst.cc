...
print_usage()
  {
    ...
    fprintf(message_out, "Usage: %s [OPTIONS...] [INFILE]\n"
      ...
      /*"      --sum                 Sum weights of duplicate strings\n"
        ...
        "Do note that %s is also guaranteed to work as epsilon always.\n"
    ...
    if (disjunct_strings)
      {
        if (use_basics)
          {
            HfstTransducer res(basic_disjunction, format);
...
int main( int argc, char **argv ) 
  {
    hfst_init_commandline(argv[0], "0.1", "Strings2Fst",
